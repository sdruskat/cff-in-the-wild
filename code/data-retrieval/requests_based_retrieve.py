...
def retrieve_cff_files(token):
    """Retrieves CITATION.cff files from GitHub via the GitHub API, using the requests library.
    Expects a directory ./data to exist.
    """
    for i in range(100):
        print(f'Getting page {i} ...')
        query_url = 'https://api.github.com/search/code?per_page=10&page=' + str(i) + '&q=filename:CITATION.cff'
        headers = {
        ...
        if response.status_code == 200:
            ...
            print(f'Not OK, Code: {response.status_code}: {response.reason} -- {response.text}')
